...
if ($env:PT__installdir) {
  $AnsibleDir = Join-Path $env:PT__installdir "lib\puppet_x\ansible_modules\ansible_role_apache_php_fpm"
} else {
  # Fallback to Puppet cache directory
  $AnsibleDir = "C:\ProgramData\PuppetLabs\puppet\cache\lib\puppet_x\ansible_modules\ansible_role_apache_php_fpm"
}
...
$CollectionPlaybook = Join-Path $AnsibleDir "roles\paw_ansible_role_apache_php_fpm\playbook.yml"
...
if ((Test-Path (Join-Path $AnsibleDir "roles")) -and (Test-Path $CollectionPlaybook)) {
  ...
  $PlaybookDir = Join-Path $AnsibleDir "roles\paw_ansible_role_apache_php_fpm"
  ...
  $result = @{
    _error = @{
      msg = "playbook.yml not found in $AnsibleDir or $AnsibleDir\roles\paw_ansible_role_apache_php_fpm"
...
try {
  ...
  if ($ExitCode -eq 0) {
    $result = @{
      status = "success"
      role = "ansible_role_apache_php_fpm"
    }
  } else {
    $result = @{
      status = "failed"
