...
# Puppet task for executing Ansible role: ansible_role_apache_php_fpm
# This script runs the entire role via ansible-playbook

# Determine the ansible modules directory
if [ -n "$PT__installdir" ]; then
  ANSIBLE_DIR="$PT__installdir/lib/puppet_x/ansible_modules/ansible_role_apache_php_fpm"
else
  # Fallback to /opt/puppetlabs/puppet/cache/lib/puppet_x/ansible_modules
  ANSIBLE_DIR="/opt/puppetlabs/puppet/cache/lib/puppet_x/ansible_modules/ansible_role_apache_php_fpm"
fi
...
if [ -d "$ANSIBLE_DIR/roles" ] && [ -f "$ANSIBLE_DIR/roles/paw_ansible_role_apache_php_fpm/playbook.yml" ]; then
  # Collection structure
  PLAYBOOK_PATH="$ANSIBLE_DIR/roles/paw_ansible_role_apache_php_fpm/playbook.yml"
  PLAYBOOK_DIR="$ANSIBLE_DIR/roles/paw_ansible_role_apache_php_fpm"
  ...
  echo "{\"_error\": {\"msg\": \"playbook.yml not found in $ANSIBLE_DIR or $ANSIBLE_DIR/roles/paw_ansible_role_apache_php_fpm\", \"kind\": \"puppet-ansible-converter/playbook-not-found\"}}"
  exit 1
fi
...
if [ $EXIT_CODE -eq 0 ]; then
  echo '{"status": "success", "role": "ansible_role_apache_php_fpm"}'
else
  echo "{\"status\": \"failed\", \"role\": \"ansible_role_apache_php_fpm\", \"exit_code\": $EXIT_CODE}"
fi
